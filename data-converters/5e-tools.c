// *******************************************************************************
// 	this will create the PF Srd monster from the bestiary file
/**********************************************************************
 *    Copyright Zen 2005+
...
compile using:
	...
	g++ 5e-tools.c -o 5e-tools ../pDBlibrary.o -I..
...
// *******************************************************************************
void copyDetails(int to, int from) {
	if (sm[to].i_space == 0) { sm[to].i_space = sm[from].i_space; }
	if (sm[to].i_size == 0) { sm[to].i_size = sm[from].i_size; }
	if (sm[to].i_armorClass[AC_NORMAL] == 0) { sm[to].i_armorClass[AC_NORMAL] = sm[from].i_armorClass[AC_NORMAL]; }
	if (sm[to].i_alignment == 0) { sm[to].i_alignment = sm[from].i_alignment; }
	if (sm[to].i_speed == 0) { sm[to].i_speed = sm[from].i_speed; }
	if (sm[to].i_initiative[0] == 0) { sm[to].i_initiative[0] = sm[from].i_initiative[0]; }
	if (sm[to].i_hp[HP_MAX] < 2) { sm[to].i_hp[HP_MAX] = sm[to].i_hp[HP_CUR] = sm[from].i_hp[HP_MAX]; }
	if (sm[to].f_challengeRating == 0) { sm[to].f_challengeRating = sm[from].f_challengeRating; }

	if (sm[to].i_abilityStats[0][ABILITY_STR] == 0) {
		sm[to].i_abilityStats[0][ABILITY_STR] = sm[from].i_abilityStats[0][ABILITY_STR];
		sm[to].i_savingThrows[SAVE_STR] = sm[from].i_savingThrows[SAVE_STR];
	}
	if (sm[to].i_abilityStats[0][ABILITY_DEX] == 0) {
		sm[to].i_abilityStats[0][ABILITY_DEX] = sm[from].i_abilityStats[0][ABILITY_DEX];
		sm[to].i_savingThrows[SAVE_DEX] = sm[from].i_savingThrows[SAVE_DEX];
	}
	if (sm[to].i_abilityStats[0][ABILITY_CON] == 0) {
		sm[to].i_abilityStats[0][ABILITY_CON] = sm[from].i_abilityStats[0][ABILITY_CON];
		sm[to].i_savingThrows[SAVE_CON] = sm[from].i_savingThrows[SAVE_CON];
	}
	if (sm[to].i_abilityStats[0][ABILITY_INT] == 0) {
		sm[to].i_abilityStats[0][ABILITY_INT] = sm[from].i_abilityStats[0][ABILITY_INT];
		sm[to].i_savingThrows[SAVE_INT] = sm[from].i_savingThrows[SAVE_INT];
	}
	if (sm[to].i_abilityStats[0][ABILITY_WIS] == 0) {
		sm[to].i_abilityStats[0][ABILITY_WIS] = sm[from].i_abilityStats[0][ABILITY_WIS];
		sm[to].i_savingThrows[SAVE_WIS] = sm[from].i_savingThrows[SAVE_WIS];
	}
	if (sm[to].i_abilityStats[0][ABILITY_CHA] == 0) {
		sm[to].i_abilityStats[0][ABILITY_CHA] = sm[from].i_abilityStats[0][ABILITY_CHA];
		sm[to].i_savingThrows[SAVE_CHA] = sm[from].i_savingThrows[SAVE_CHA];
	}
	sm[to].i_skills[SKILL_CONCENTRATION] = sm[from].i_skills[SKILL_CONCENTRATION];
	sm[to].i_skills[SKILL_SPOT] = sm[from].i_skills[SKILL_SPOT];
	sm[to].i_skills[SKILL_MOVESILENTLY] = sm[from].i_skills[SKILL_MOVESILENTLY];
	sm[to].i_skills[SKILL_SOH] = sm[from].i_skills[SKILL_SOH];

	if (sm[to].w_weapons[0].c_description[0] == '\0' && sm[to].w_weapons[1].c_description[0] == '\0') {
		sm[to].i_noAttacks = sm[from].i_noAttacks;
		memcpy(&sm[to].w_weapons[0].c_available, &sm[from].w_weapons[0].c_available, sizeof(ADMPweapon) * MAX_ATTACKS);
	}

// how to copy any notes from the other monster
...
int main(int argc, char **argv) {
	...
	//char ofn[] = "sdarkmonster", notefile[30];
	char ofn[] = "srdv55monster", notefile[30];

	sprintf(notefile, "%s.snt", ofn);

	memset(&sm[0].c_name[0], '\0', sizeof(ADMPplayer)*(MAX_QKMONSTERS+83));

	if ((nof = fopen(notefile, "w+")) == NULL) {
...
#include "5e-tools-2024.h"
//#include "shadowdark.h"
fputs("\n", nof);
...
//		l = i;
		if (sm[i].c_name[0] != '\0') { // && sm[i].i_hp[HP_MAX] > 0) {
			...
			if (sm[i].w_weapons[0].c_description[0] == '\0') {
				printf("%s has no weapons ...\n", sm[i].c_name);
			}
		}
	}
	return 0;
