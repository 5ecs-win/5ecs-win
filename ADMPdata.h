...
#define PENALTY_EXHAUSTION		1		// d100
...
#define DICE_RMS				2
#define DICE_D20V5			3
#define DICE_PATHFINDER		4
#define DICE_PF2E				5
#define DICE_D20V55			6
#define DICE_SDARK			7
...
typedef struct ADMPweapon {
	...
	char	c_message[80];			/* a message to display about this weapon or attack */
...
typedef struct ADMPflags {
	...
	uchar	f_massUnit:1;
	uchar	f_massFriendly:1;
	uchar	f_massStatus:2;
	uchar	f_displayed:1;
} ADMPflags;
...
typedef struct ADMPflags3 {
	...
	uchar	f_forgetLastAttacked:1;
	uchar	f_showHealthStatus:1;
	uchar	f_autoCentrePlayerWindow:1;
	uchar	f_flag16:1;
} ADMPflags3;

typedef struct ADMPconditions {
	uchar f_blinded:1;
	uchar f_charmed:1;
	uchar f_deafened:1;
	uchar f_exhaustion:1;
	uchar f_frightened:1;
	uchar f_grappled:1;
	uchar f_incapacitated:1;
	uchar f_invisible:1;
	uchar f_paralyzed:1;
	uchar f_petrified:1;
	uchar f_poisoned:1;
	uchar f_prone:1;
	uchar f_restrained:1;
	uchar f_stunned:1;
	uchar f_unconscious:1;
} ADMPconditions;
...
typedef struct ADMPplayer {
	...
	int	i_xp[2];
	myuint c_d100XP[MAX_D100XP];						// used for d100 XP calcs
	...
	int	i_health;										// health percentage
	...
	int	i_monsterType;									// 0 = normal, 1 = SRD 5.0, 2 = SRD 5.2
	...
	char	c_inactiveIconFile[160];			// the icon to associate with it
	char	c_iconFile[160];			// the icon to associate with it
	char	c_items[190];			// any special items they carry

	char	i_iconType;				// is the icon type png/gif/jpg/animated
	char	i_altFormID;			// alternate form ID ... player ID
	sint	i_noInGroup;			// used for mass combats

	int	i_tempXP;				// XP gained in this combat d100/MERP systems

	int	i_spellPoints[2];		// use spell points instead of slots

	//ADMPconditions conditions;
...
typedef struct ADMPobject {
	...
	sint	i_id;
	char	c_iconFile[160];			// the icon to associate with it
	char	c_playericonFile[160];	// the icon to associate with it
	sint  i_width, i_height;
	sint  i_gridX, i_gridY, i_gridSize;
	sint	i_type;
	sint	i_drawn;
	uchar f_hidden;
	uchar i_mirror;
...
typedef struct ADMPquickmods {
	sint	i_id;
	sint	i_effects[MAX_QUICKMODTYPES];			// BAB, AC, Dam, HP, Init, All Saves [effects-type, value], Miss%, SR
	char	c_value[MAX_QUICKMODTYPES][6];			// BAB, AC, Dam, HP, Init, All Saves [effects-type, value], Miss%, SR
	sint	i_noRounds;										// how many rounds to last, 0 = forever, > 0 will count down to 0
} ADMPquickmods;
